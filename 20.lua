...
local function BuildUI()
    ...
    -- VFX-Based Event Detection System (More Accurate)
    local EventDetection = {
        activeEvents = {},
        lastEventCheck = 0,
        checkInterval = 15, -- Check every 15 seconds for better responsiveness
        
        -- VFX patterns untuk detection yang lebih akurat
        vfxPatterns = {
            ["Christmas Event"] = {
                vfx = {"Enchant Glow", "Ghost Shark Obtained"},
            ...
            ["Group Milestone Event"] = {
                vfx = {"Shark Obtained"},
                location = CFrame.new(45.2788086, 252.562927, 2987.10913),
                checkFunction = function()
                    -- Check for group milestone indicators
                    return ReplicatedStorage:FindFirstChild("Controllers") and
                           ReplicatedStorage.Controllers:FindFirstChild("GroupRewardController") ~= nil
                end
            },
            ["Double Luck Event"] = {
                vfx = {"Abyssal Chroma Dive"},
            ...
            ["Code Redemption"] = {
                vfx = {"Ghost Shark Obtained"},
                location = CFrame.new(-650.971191, 208.693695, 711.10907),
                checkFunction = function()
                    -- Check for code system
                    return ReplicatedStorage:FindFirstChild("Controllers") and
                           ReplicatedStorage.Controllers:FindFirstChild("CodeController") ~= nil
                end
            }
        }
    }

    -- Enhanced VFX-Based Event Detection
    local function DetectActiveEventsVFX()
        local events = {}
        local now = tick()
        
        -- Scan all VFX patterns
        for eventName, eventData in pairs(EventDetection.vfxPatterns) do
            ...
            if not isActive and eventData.checkFunction then
                isActive = eventData.checkFunction()
            end
            ...
            if isActive then
                local eventInfo = {
                    ...
                    type = "vfx_detected",
                    description = "Detected via VFX analysis"
                }
                
                -- Add specific descriptions
                if eventName:find("Christmas") then
                    eventInfo.name = "🎄 Christmas Event"
                    eventInfo.description = "Christmas VFX & special spawns detected"
                elseif eventName:find("Group") then
                    eventInfo.name = "🎯 Group Milestone Event"
                    eventInfo.description = "Group rewards active - join Fisch group!"
                elseif eventName:find("Double Luck") then
                    eventInfo.name = "🍀 Double Luck Event"
                    eventInfo.description = "Rare fish VFX & double luck detected"
                elseif eventName:find("Weather") then
                    eventInfo.name = "🌤️ Weather Event System"
                    eventInfo.description = "Weather VFX & custom events detected"
                elseif eventName:find("Code") then
                    eventInfo.name = "🎫 Code Redemption"
                    eventInfo.description = "Code system active & VFX present"
                end
    ...
    eventSection.Position = UDim2.new(0, 0, 1, -210)
    eventSection.BackgroundColor3 = Color3.fromRGB(25,25,32)
    eventSection.BorderSizePixel = 0
    Instance.new("UICorner", eventSection)

    local eventTitle = Instance.new("TextLabel", eventSection)
    eventTitle.Size = UDim2.new(1, -20, 0, 30)
    ...
    eventScrollFrame.Position = UDim2.new(0, 5, 0, 35)
    ...
    local function UpdateEventDisplay()
        -- Clear existing event buttons
        for _, child in pairs(eventScrollFrame:GetChildren()) do
            if child:IsA("TextButton") then
                child:Destroy()
            end
        end
        
        local events = DetectActiveEventsVFX() -- Use new VFX detection
        local eventYOffset = 5
        
        if #events == 0 then
            local noEventsLabel = Instance.new("TextLabel", eventScrollFrame)
            noEventsLabel.Size = UDim2.new(1, -10, 0, 30)
            noEventsLabel.Position = UDim2.new(0, 5, 0, eventYOffset)
            noEventsLabel.Text = "🔍 No VFX events detected - Scanning..."
            noEventsLabel.Font = Enum.Font.Gotham
            noEventsLabel.TextSize = 12
            noEventsLabel.TextColor3 = Color3.fromRGB(180,180,180)
            noEventsLabel.BackgroundTransparency = 1
            noEventsLabel.TextXAlignment = Enum.TextXAlignment.Left
        else
            for _, event in pairs(events) do
                local eventBtn = Instance.new("TextButton", eventScrollFrame)
                eventBtn.Size = UDim2.new(1, -10, 0, 35)
                eventBtn.Position = UDim2.new(0, 5, 0, eventYOffset)
                eventBtn.Text = event.name .. " ✨"
                eventBtn.Font = Enum.Font.GothamSemibold
                eventBtn.TextSize = 11
                eventBtn.BackgroundColor3 = Color3.fromRGB(70,70,85) -- Slightly different color for VFX
                eventBtn.TextColor3 = Color3.fromRGB(255,255,255)
                eventBtn.BorderSizePixel = 0
                Instance.new("UICorner", eventBtn)
                
                local eventDesc = Instance.new("TextLabel", eventBtn)
                eventDesc.Size = UDim2.new(1, -10, 0, 15)
                eventDesc.Position = UDim2.new(0, 5, 1, -18)
                eventDesc.Text = event.description .. " (VFX Detected)"
...
_G.ModernAutoFish = {
    ...
    DetectEventsVFX = function()
        return DetectActiveEventsVFX()
    end,
    TeleportToEvent = function(eventName)
        local events = DetectActiveEventsVFX() -- Use VFX detection
        for _, event in pairs(events) do
            if event.name:find(eventName) then
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    LocalPlayer.Character.HumanoidRootPart.CFrame = event.location
                    return "Teleported to " .. event.name .. " (VFX Detected)"
                end
                return "Character not found"
            end
        end
        return "Event not found: " .. eventName .. " (No VFX detected)"
    end,
    GetEventStatus = function()
        local events = DetectActiveEventsVFX() -- Use VFX detection
        local status = {
            activeEventCount = #events,
            detectionMethod = "VFX-Based",
            events = {}
        }
        for _, event in pairs(events) do
            table.insert(status.events, {
                name = event.name,
                type = event.type,
                description = event.description,
                detectionMethod = "VFX Analysis"
            })
        end
        return status
    end,
    GetVFXStatus = function()
        local vfxList = {}
        if workspace:FindFirstChild("CosmeticFolder") then
            for _, vfx in pairs(workspace.CosmeticFolder:GetChildren()) do
...
_G.TELEPORT_TO_EVENT = function(eventName)
    if not eventName then
        print("Usage: _G.TELEPORT_TO_EVENT('Christmas') or _G.TELEPORT_TO_EVENT('Group')")
        return
    end
