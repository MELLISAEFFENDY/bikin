...
-- MODIFIED: Integrated Custom Auto Sell feature
...
-- ====================================================================
-- KODE BARU: Variabel untuk Auto Sell
-- ====================================================================
local autoSellCustomThreshold = 20 -- Nilai default jumlah ikan untuk dijual
local autoSellCustomEnabled = false -- Status default untuk fitur ini
local isCurrentlySelling = false -- Mencegah loop penjualan ganda
-- ====================================================================
...
-- ====================================================================
-- KODE BARU: Fungsi Auto Sell
-- ====================================================================
local function CheckAndAutoSell()
    -- Fungsi ini sekarang menggunakan variabel dari input dan toggle yang baru
    if autoSellCustomEnabled and not isCurrentlySelling and Dashboard.sessionStats.fishCount >= autoSellCustomThreshold then
        isCurrentlySelling = true -- Tandai bahwa proses penjualan dimulai
        pcall(function()
            ...
            -- Mencari NPC penjual (Alex) atau NPC toko lainnya
            local sellNpc = nil
            ...
            if not sellNpc then
                Notify("Auto Sell", "NPC Penjual tidak ditemukan! Mencoba teleport ke lokasi toko...")
                -- Fallback: Teleport ke koordinat toko yang diketahui jika NPC tidak ditemukan
            ...
            -- Teleport ke NPC, jual, dan kembali
            Notify("Auto Sell", "Teleport ke toko untuk menjual " .. Dashboard.sessionStats.fishCount .. " ikan...")
            wait(1.5)

            local sellRemote = ResolveRemote("RF/SellAllItems")
            if sellRemote then
                 safeInvoke(sellRemote)
            else
                Notify("Auto Sell", "Error: Remote untuk menjual tidak ditemukan.")
                isCurrentlySelling = false
                return
            end
            ...
            Notify("Auto Sell", "Penjualan berhasil! Kembali memancing.")
            isCurrentlySelling = false -- Tandai proses penjualan selesai
        end)
    end
end
-- ====================================================================
...
local function LogFishCatch(fishName, location)
    ...
    -- ====================================================================
    -- KODE BARU: Panggil fungsi CheckAndAutoSell setelah ikan ditangkap
    -- ====================================================================
    CheckAndAutoSell()
    -- ====================================================================
...
local function BuildUI()
    ...
    -- ====================================================================
    -- KODE BARU: UI untuk Auto Sell
    -- ====================================================================
    local autoSellSection = Instance.new("Frame", fishingAIScrollFrame)
    autoSellSection.Size = UDim2.new(1, -10, 0, 100)
    autoSellSection.Position = UDim2.new(0, 5, 0, 465) -- Ditempatkan setelah AntiAFK
    ...
    autoSellTitle.Text = "💰 Auto Sell Settings"
    ...
    local autoSellInput = Instance.new("TextBox", autoSellSection)
    autoSellInput.Size = UDim2.new(0.5, -15, 0, 28)
    autoSellInput.Position = UDim2.new(0, 10, 0, 35)
    autoSellInput.PlaceholderText = "Jumlah Ikan"
    autoSellInput.Text = tostring(autoSellCustomThreshold)
    autoSellInput.Font = Enum.Font.GothamSemibold
    autoSellInput.TextSize = 12
    autoSellInput.BackgroundColor3 = Color3.fromRGB(60,60,66)
    autoSellInput.TextColor3 = Color3.fromRGB(255,255,255)
    autoSellInput.BorderSizePixel = 0
    Instance.new("UICorner", autoSellInput)

    local autoSellToggle = Instance.new("TextButton", autoSellSection)
    autoSellToggle.Size = UDim2.new(0.5, -15, 0, 28)
    autoSellToggle.Position = UDim2.new(0.5, 5, 0, 35)
    ...
    autoSellStatus.Size = UDim2.new(1, -20, 0, 20)
    autoSellStatus.Position = UDim2.new(0, 10, 0, 70)
    autoSellStatus.Text = "Status: Dinonaktifkan"
    ...
    -- Fungsionalitas UI Auto Sell
    autoSellInput.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            local num = tonumber(autoSellInput.Text)
            if num and num > 0 then
                autoSellCustomThreshold = num
                Notify("Auto Sell", "Threshold diatur ke: " .. num .. " ikan")
                if autoSellCustomEnabled then
                    autoSellStatus.Text = "Status: Aktif, akan menjual pada " .. autoSellCustomThreshold .. " ikan"
                end
            else
                Notify("Input Error", "Harap masukkan angka yang valid.")
                autoSellInput.Text = tostring(autoSellCustomThreshold) -- Kembalikan ke nilai lama jika tidak valid
            end
        end
    end)

    autoSellToggle.MouseButton1Click:Connect(function()
        autoSellCustomEnabled = not autoSellCustomEnabled
        if autoSellCustomEnabled then
            ...
            Notify("Auto Sell", "Auto sell diaktifkan!")
        else
            autoSellToggle.Text = "🔴 OFF"
            autoSellToggle.BackgroundColor3 = Color3.fromRGB(160,60,60)
            autoSellStatus.Text = "Status: Dinonaktifkan"
            autoSellStatus.TextColor3 = Color3.fromRGB(200,200,200)
            Notify("Auto Sell", "Auto sell dinonaktifkan.")
        end
    end)
    -- ====================================================================
    ...
    futureSection.Position = UDim2.new(0, 5, 0, 575) -- Disesuaikan posisinya
    ...
    fishingAIScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 680) -- Ukuran canvas disesuaikan
