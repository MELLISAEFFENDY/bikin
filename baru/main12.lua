...
-- Auto Sell Remotes
local sellItemRemote = ResolveRemote("RF/SellItem")
...
local AutoSell = {
    ...
    sellRare = true,
    sellAtPercent = 90,
    sellAtFishCount = 100, -- Sell after catching 100 fish
    lastSellFishCount = 0, -- Tracks fish count at last sale
    isSelling = false,
    sessionId = 0
}
...
local function SellItemsByRarity()
    ...
    for _, item in pairs(inventoryData.Items) do
        if item and item.Name then
            ...
            if (AutoSell.sellCommon and rarity == "COMMON") or
               (AutoSell.sellUncommon and rarity == "UNCOMMON") or
               (AutoSell.sellRare and rarity == "RARE") then
    ...
    if #itemsToSell == 0 then
        Notify("Auto Sell", "✅ No items to sell.")
        AutoSell.isSelling = false
        return
    end

    Notify("Auto Sell", "Found " .. #itemsToSell .. " items to sell. Selling now...")
    local soldCount = 0
    ...
    Notify("Auto Sell", "✅ Sold " .. soldCount .. " items.")
    AutoSell.isSelling = false
end

local function AutoSellRunner(mySessionId)
    Notify("Auto Sell", "🤖 Auto Sell system started.")
    while AutoSell.enabled and AutoSell.sessionId == mySessionId do
        ...
        if currentItems and maxItems and maxItems > 0 then
            ...
            if percentage >= AutoSell.sellAtPercent then
                shouldSell = true
                reason = string.format("Inventory at %.0f%%.", percentage)
            end
        end
        
        -- Check 2: By Fish Count (only if not already triggered by percentage)
        if not shouldSell then
            local fishSinceLastSell = Dashboard.sessionStats.fishCount - AutoSell.lastSellFishCount
            if fishSinceLastSell >= AutoSell.sellAtFishCount then
                shouldSell = true
                reason = string.format("%d fish caught since last sell.", fishSinceLastSell)
            end
        end

        -- Trigger sell if any condition is met
        if shouldSell then
            Notify("Auto Sell", reason .. " Triggering sell.")
            SellItemsByRarity()
            AutoSell.lastSellFishCount = Dashboard.sessionStats.fishCount -- Update tracker after selling
        end
        
        task.wait(10) -- Check every 10 seconds
    end
    Notify("Auto Sell", "🤖 Auto Sell system stopped.")
end
...
local function BuildUI()
    ...
    -- NEW: Auto Sell Section
    local autoSellSection = Instance.new("Frame", fishingAIScrollFrame)
    autoSellSection.Size = UDim2.new(1, -10, 0, 220) -- Increased height
    autoSellSection.Position = UDim2.new(0, 5, 0, 395)
    ...
    autoSellTitle.Text = "💰 Smart Auto Sell"
    autoSellTitle.Font = Enum.Font.GothamBold
    autoSellTitle.TextSize = 14
    autoSellTitle.TextColor3 = Color3.fromRGB(255, 190, 0)
    ...
    autoSellToggle.Size = UDim2.new(0, 70, 0, 24)
    autoSellToggle.Position = UDim2.new(1, -80, 0, 5)
    ...
    local sellPercentLabel = Instance.new("TextLabel", autoSellSection)
    sellPercentLabel.Size = UDim2.new(1, -20, 0, 20)
    sellPercentLabel.Position = UDim2.new(0, 10, 0, 35)
    sellPercentLabel.Text = "Jual saat inventaris: 90%"
    sellPercentLabel.Font = Enum.Font.GothamSemibold
    sellPercentLabel.TextSize = 12
    sellPercentLabel.TextColor3 = Color3.fromRGB(200,200,200)
    sellPercentLabel.BackgroundTransparency = 1
    sellPercentLabel.TextXAlignment = Enum.TextXAlignment.Left

    local sellSlider = Instance.new("Frame", autoSellSection)
    sellSlider.Size = UDim2.new(1, -20, 0, 16)
    sellSlider.Position = UDim2.new(0, 10, 0, 60)
    sellSlider.BackgroundColor3 = Color3.fromRGB(30,30,35)
    Instance.new("UICorner", sellSlider)
    local sellFill = Instance.new("Frame", sellSlider)
    sellFill.Size = UDim2.new(0.9, 0, 1, 0)
    sellFill.BackgroundColor3 = Color3.fromRGB(255, 190, 0)
    Instance.new("UICorner", sellFill)
    local sellHandle = Instance.new("TextButton", sellSlider)
    sellHandle.Size = UDim2.new(0, 16, 1, 0)
    sellHandle.Position = UDim2.new(0.9, -8, 0, 0)
    sellHandle.BackgroundColor3 = Color3.fromRGB(255,255,255)
    Instance.new("UICorner", sellHandle)

    -- NEW: Sell by Fish Count UI
    local sellCountLabel = Instance.new("TextLabel", autoSellSection)
    sellCountLabel.Size = UDim2.new(0.6, 0, 0, 25)
    sellCountLabel.Position = UDim2.new(0, 10, 0, 85)
    sellCountLabel.Text = "Jual setiap (ikan):"
    sellCountLabel.Font = Enum.Font.GothamSemibold
    sellCountLabel.TextSize = 12
    sellCountLabel.TextColor3 = Color3.fromRGB(200,200,200)
    sellCountLabel.BackgroundTransparency = 1
    sellCountLabel.TextXAlignment = Enum.TextXAlignment.Left

    local sellCountInput = Instance.new("TextBox", autoSellSection)
    sellCountInput.Size = UDim2.new(0.4, -20, 0, 25)
    sellCountInput.Position = UDim2.new(0.6, 10, 0, 85)
    sellCountInput.Text = tostring(AutoSell.sellAtFishCount)
    sellCountInput.Font = Enum.Font.GothamSemibold
    sellCountInput.TextSize = 12
    sellCountInput.BackgroundColor3 = Color3.fromRGB(30,30,35)
    sellCountInput.TextColor3 = Color3.fromRGB(255,255,255)
    sellCountInput.ClearTextOnFocus = false
    Instance.new("UICorner", sellCountInput)

    local rarityTogglesFrame = Instance.new("Frame", autoSellSection)
    rarityTogglesFrame.Size = UDim2.new(1, -20, 0, 80)
    rarityTogglesFrame.Position = UDim2.new(0, 10, 0, 120) -- Moved down
    rarityTogglesFrame.BackgroundTransparency = 1

    local function createRarityToggle(text, color, position, configKey)
        local btn = Instance.new("TextButton", rarityTogglesFrame)
        btn.Size = UDim2.new(0.32, 0, 0, 25)
        btn.Position = position
        btn.Text = text
        btn.Font = Enum.Font.GothamBold
        btn.TextSize = 11
        btn.BackgroundColor3 = AutoSell[configKey] and color or Color3.fromRGB(80,80,80)
        btn.TextColor3 = Color3.fromRGB(255,255,255)
        Instance.new("UICorner", btn)
        
        btn.MouseButton1Click:Connect(function()
            AutoSell[configKey] = not AutoSell[configKey]
            btn.BackgroundColor3 = AutoSell[configKey] and color or Color3.fromRGB(80,80,80)
        end)
        return btn
    end

    createRarityToggle("SELL COMMON", Color3.fromRGB(100,100,100), UDim2.new(0, 0, 0, 0), "sellCommon")
    createRarityToggle("SELL UNCOMMON", Color3.fromRGB(70,150,90), UDim2.new(0.34, 0, 0, 0), "sellUncommon")
    createRarityToggle("SELL RARE", Color3.fromRGB(80,120,180), UDim2.new(0.68, 0, 0, 0), "sellRare")
    ...
    antiAfkSection.Position = UDim2.new(0, 5, 0, 625) -- Adjusted position
    ...
    fishingAIScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 695) -- Increased canvas size
    ...
    weatherSection.Position = UDim2.new(0, 5, 0, 575) -- Moved down to accommodate reconnect section
    ...
    featureScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 720) -- Increased to accommodate new sections
    ...
    -- NEW: Auto Sell slider functionality
    local draggingSell = false
    sellHandle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then draggingSell = true end
    end)
    UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then draggingSell = false end
    end)
    UserInputService.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement and draggingSell then
            local relativeX = input.Position.X - sellSlider.AbsolutePosition.X
            local percentage = math.clamp(relativeX / sellSlider.AbsoluteSize.X, 0, 1)
            AutoSell.sellAtPercent = math.floor(percentage * 100)
            sellPercentLabel.Text = "Jual saat inventaris: " .. AutoSell.sellAtPercent .. "%"
            sellFill.Size = UDim2.new(percentage, 0, 1, 0)
            sellHandle.Position = UDim2.new(percentage, -8, 0, 0)
        end
    end)

    -- NEW: Auto Sell by Fish Count input
    sellCountInput.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            local num = tonumber(sellCountInput.Text)
            if num and num > 0 then
                AutoSell.sellAtFishCount = num
                Notify("Auto Sell", "Sell count set to: " .. num)
            else
                sellCountInput.Text = tostring(AutoSell.sellAtFishCount) -- Revert if invalid
                Notify("Auto Sell", "Invalid number for sell count.")
            end
        end
    end)

    -- NEW: Auto Sell toggle
    autoSellToggle.MouseButton1Click:Connect(function()
        ...
        if AutoSell.enabled then
            ...
            AutoSell.sessionId = AutoSell.sessionId + 1
            AutoSell.lastSellFishCount = Dashboard.sessionStats.fishCount -- Reset tracker on start
            task.spawn(function() AutoSellRunner(AutoSell.sessionId) end)
        else
            autoSellToggle.Text = "🔴 OFF"
            autoSellToggle.BackgroundColor3 = Color3.fromRGB(160,60,60)
            AutoSell.sessionId = AutoSell.sessionId + 1
