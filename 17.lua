...
local function BuildUI()
    ...
    -- Event Detection & Teleportation System
    local EventDetection = {
        activeEvents = {},
        lastEventCheck = 0,
        checkInterval = 30, -- Check every 30 seconds
        eventLocations = {
            ["Christmas Event"] = CFrame.new(-1551.25, 2.87, 1920.26),
            ["Weather Event"] = CFrame.new(-1488.51196, 83.1732635, 1876.30298),
            ["Double Luck Event"] = CFrame.new(-650.971191, 208.693695, 711.10907),
            ["Group Milestone"] = CFrame.new(45.2788086, 252.562927, 2987.10913),
            ["Special Spawn Event"] = CFrame.new(1944.77881, 393.562927, 1371.35913),
            ["Admin Event"] = CFrame.new(-2095.34106, 197.199997, 3718.08008),
            ["Code Redemption"] = CFrame.new(-650.971191, 208.693695, 711.10907)
        }
    }
    ...
    local function DetectActiveEvents()
        ...
        if ReplicatedStorage:FindFirstChild("Shared") and 
           ReplicatedStorage.Shared:FindFirstChild("ChristmasEventEndTime") then
            local success, endTime = pcall(function()
                return require(ReplicatedStorage.Shared.ChristmasEventEndTime)
            end)
            if success and endTime and now < endTime then
                table.insert(events, {
                    name = "🎄 Christmas Event",
                    location = EventDetection.eventLocations["Christmas Event"],
                    description = "Christmas items & rewards available",
                    type = "seasonal"
                })
            end
        end
        ...
        -- Check Group Rewards
        if ReplicatedStorage:FindFirstChild("Controllers") and
           ReplicatedStorage.Controllers:FindFirstChild("GroupRewardController") then
            table.insert(events, {
                name = "🎯 Group Milestone Event",
                location = EventDetection.eventLocations["Group Milestone"],
                description = "Group rewards active - join Fisch group!",
                type = "group"
            })
        end
        
        -- Check Double Luck Products
        if ReplicatedStorage:FindFirstChild("Shared") and
           ReplicatedStorage.Shared:FindFirstChild("Products") and
           ReplicatedStorage.Shared.Products:FindFirstChild("DoubleLuckProducts") then
            table.insert(events, {
                name = "🍀 Double Luck Event",
                location = EventDetection.eventLocations["Double Luck Event"],
                description = "Double luck items available for purchase",
                type = "shop"
            })
        end
        
        -- Check Weather Events
        if purchaseWeatherEventRemote then
            table.insert(events, {
                name = "🌤️ Weather Event System",
                location = EventDetection.eventLocations["Weather Event"],
                description = "Custom weather events can be purchased",
                type = "weather"
            })
        end
        ...
        -- Check Code System
        if ReplicatedStorage:FindFirstChild("Controllers") and
           ReplicatedStorage.Controllers:FindFirstChild("CodeController") then
            table.insert(events, {
                name = "🎫 Code Redemption",
                location = EventDetection.eventLocations["Code Redemption"],
                description = "Code redemption system active",
                type = "codes"
            })
        end
    ...
    eventTitle.Text = "🎪 Active Events & Special Locations"
    ...
    local function UpdateEventDisplay()
        ...
        if #events == 0 then
            ...
            for _, event in pairs(events) do
                ...
                eventBtn.BackgroundColor3 = Color3.fromRGB(60,60,70)
    ...
    TeleportToEvent = function(eventName)
        ...
        for _, event in pairs(events) do
            if event.name:find(eventName) then
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    LocalPlayer.Character.HumanoidRootPart.CFrame = event.location
                    return "Teleported to " .. event.name
                end
