...
-- Added XSAN Event Detector System
...
-- ====================================================================
-- XSAN EVENT DETECTOR SYSTEM (DISABLED FOR TESTING)
-- ====================================================================
...
-- Minimal EventDetector for testing
local EventDetector = {
    detectedEvents = {},
    eventLocations = {},
    adminEventsList = {},
...
-- Event Location Scanner (STUB VERSION)
function ScanEventLocations()
    print("XSAN: ScanEventLocations called (stub)")
end
...
-- Debug information (COMMENTED OUT FOR TESTING)
--[[
...
function ScanEventLocations()
    print("XSAN: ScanEventLocations stub")
end

function TeleportToEvent(eventName)
    print("XSAN: TeleportToEvent stub for:", eventName)
    Notify("Event Teleport", "Test teleport to " .. tostring(eventName))
end

print("XSAN: Basic functions loaded")
...
local function BuildUI()
    ...
    eventScrollFrame.Size = UDim2.new(1, 0, 1, -30)
    eventScrollFrame.Position = UDim2.new(0, 0, 0, 30)
    ...
    local eventStatusSection = Instance.new("Frame", eventScrollFrame)
    eventStatusSection.Size = UDim2.new(1, -10, 0, 100)
    eventStatusSection.Position = UDim2.new(0, 5, 0, 5)
    ...
    eventScannerStatus.Size = UDim2.new(1, -20, 0, 60)
    ...
    eventScannerStatus.TextSize = 12
    ...
    local detectedEventsSection = Instance.new("Frame", eventScrollFrame)
    detectedEventsSection.Size = UDim2.new(1, -10, 0, 200)
    detectedEventsSection.Position = UDim2.new(0, 5, 0, 115)
    detectedEventsSection.BackgroundColor3 = Color3.fromRGB(45,45,52)
    detectedEventsSection.BorderSizePixel = 0
    Instance.new("UICorner", detectedEventsSection)

    local detectedEventsTitle = Instance.new("TextLabel", detectedEventsSection)
    detectedEventsTitle.Size = UDim2.new(1, -20, 0, 25)
    detectedEventsTitle.Position = UDim2.new(0, 10, 0, 5)
    detectedEventsTitle.Text = "🚨 Detected Events"
    detectedEventsTitle.Font = Enum.Font.GothamBold
    detectedEventsTitle.TextSize = 14
    detectedEventsTitle.TextColor3 = Color3.fromRGB(255,100,100)
    detectedEventsTitle.BackgroundTransparency = 1
    detectedEventsTitle.TextXAlignment = Enum.TextXAlignment.Left

    local eventsListFrame = Instance.new("ScrollingFrame", detectedEventsSection)
    eventsListFrame.Size = UDim2.new(1, -20, 1, -35)
    eventsListFrame.Position = UDim2.new(0, 10, 0, 30)
    eventsListFrame.BackgroundColor3 = Color3.fromRGB(35,35,40)
    eventsListFrame.BorderSizePixel = 0
    eventsListFrame.ScrollBarThickness = 4
    eventsListFrame.ScrollBarImageColor3 = Color3.fromRGB(80,80,80)
    Instance.new("UICorner", eventsListFrame)

    -- Function to update events list
    local function UpdateEventsList()
        -- Clear existing event items
        for _, child in pairs(eventsListFrame:GetChildren()) do
        ...
        for eventName, eventInfo in pairs(EventDetector.detectedEvents) do
            if eventInfo.detected then
                ...
                local eventData = EventDetector.adminEventsList[eventName]
                
                -- Create event item frame
                local eventItem = Instance.new("Frame", eventsListFrame)
                eventItem.Size = UDim2.new(1, -10, 0, 70)
                ...
                eventNameLabel.Size = UDim2.new(0.6, -10, 0, 25)
                ...
                -- Event rarity
                local eventRarity = Instance.new("TextLabel", eventItem)
                eventRarity.Size = UDim2.new(0.35, -5, 0, 20)
                eventRarity.Position = UDim2.new(0.65, 0, 0, 5)
                eventRarity.Text = "⭐ " .. eventData.rarity
                eventRarity.Font = Enum.Font.GothamSemibold
                eventRarity.TextSize = 11
                
                -- Color by rarity
                if eventData.rarity == "MYTHIC" then
                    eventRarity.TextColor3 = Color3.fromRGB(255,0,255)
                elseif eventData.rarity == "LEGENDARY" then
                    eventRarity.TextColor3 = Color3.fromRGB(255,215,0)
                elseif eventData.rarity == "EPIC" then
                    eventRarity.TextColor3 = Color3.fromRGB(128,0,255)
                else
                    eventRarity.TextColor3 = Color3.fromRGB(100,255,100)
                end
                
                eventRarity.BackgroundTransparency = 1
                eventRarity.TextXAlignment = Enum.TextXAlignment.Right

                -- Event description
                local eventDesc = Instance.new("TextLabel", eventItem)
                eventDesc.Size = UDim2.new(1, -20, 0, 20)
                eventDesc.Position = UDim2.new(0, 10, 0, 25)
                eventDesc.Text = eventData.description
                eventDesc.Font = Enum.Font.Gotham
                eventDesc.TextSize = 10
                ...
                teleportBtn.Position = UDim2.new(1, -90, 0, 45)
                ...
                teleportBtn.MouseEnter:Connect(function()
                    teleportBtn.BackgroundColor3 = Color3.fromRGB(90,150,255)
                end)
        ...
        -- Update canvas size
        eventsListFrame.CanvasSize = UDim2.new(0, 0, 0, math.max(yPos, 100))

        -- Update no events message
        if eventCount == 0 then
            local noEventsLabel = Instance.new("TextLabel", eventsListFrame)
            noEventsLabel.Size = UDim2.new(1, -20, 0, 30)
    ...
    -- Update events list every 2 seconds
    spawn(function()
        while true do
            UpdateEventsList()
            wait(2)
    ...
    -- Manual scan button
    local manualScanBtn = Instance.new("TextButton", eventScrollFrame)
    manualScanBtn.Size = UDim2.new(0, 120, 0, 30)
    manualScanBtn.Position = UDim2.new(0, 10, 0, 325)
    ...
    eventScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 370)

    -- Test detection button
    local testDetectionBtn = Instance.new("TextButton", eventScrollFrame)
    testDetectionBtn.Size = UDim2.new(0, 120, 0, 30)
    testDetectionBtn.Position = UDim2.new(0, 140, 0, 325)
    ...
    testDetectionBtn.MouseButton1Click:Connect(function()
        ...
        EventDetector.detectedEvents["Black Hole"] = {
            startTime = tick(),
            detected = true,
            location = CFrame.new(-1000, 200, -1000),
            gui = nil
        }
        EventDetector.eventLocations["Black Hole"] = CFrame.new(-1000, 200, -1000)
        Notify("🧪 TEST EVENT", "🕳️ Black Hole (TEST) detected!")
        UpdateEventsList()
    end)
